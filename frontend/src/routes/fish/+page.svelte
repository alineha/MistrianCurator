...
<script lang="ts">
    ...
    function updatePercentage(){
        ...
        if(museuminfo != null)
        {
            getSetPercentage('fish', museuminfo);
        }
    }
    ...
    onMount(async () => {
		bundles = await getBundles("fish");
        bundlesNumber = bundles.length-1;
        for (var bundle of bundles)
        {
            let itemsOfBundle = await getBundleItems("fish", bundle);
...
<main>
    <center><div class="title">
      ...
      <img class="rightBorder" src={borderTitle} alt="Border Left">
        ...
        <center><h1>FISH: {categoriesPercentages['fish']}% COMPLETE</h1></center>

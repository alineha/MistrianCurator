...
<script lang="ts">
    ...
    function updatePercentage(){
        ...
        if(museuminfo != null)
        {
            getSetPercentage('insects', museuminfo);
        }
    }
    ...
    onMount(async () => {
		bundles = await getBundles("insects");
        bundlesNumber = bundles.length-1;
        for (var bundle of bundles)
        {
            let itemsOfBundle = await getBundleItems("insects", bundle);
...
<main>
    <center><div class="title">
      ...
      <img class="rightBorder" src={borderTitle} alt="Border Left">
        ...
        <center><h1>INSECTS: {categoriesPercentages['insects']}% COMPLETE</h1></center>
